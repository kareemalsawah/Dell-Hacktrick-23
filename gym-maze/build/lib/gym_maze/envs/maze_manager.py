...
class MazeManager:
    def __init__(self, maze_size=10):
        ...
        self.riddles_dict = (
            dict()
        )  ##### dictionary to map agent id to RiddleContainer object
    ...
    def init_maze(self, agent_id, maze_cells=None):
        if hasattr(maze_cells, "shape"):
            # print("hey" * 20, self.rescue_items_dict)
            env = gym.make(
                "maze-sample-10x10-v0",
                rescue_item_locations=list(self.rescue_items_dict.keys()),
        ...
        if riddle_type == actual_riddle_type:
            ...
            if not riddle.solved():
                ...
                if riddle.solved():
                    ...
                    # print("riddle solved")
                    if (
                        not self.maze_map[agent_id]
                        .maze_view.maze.get_rescue_item(tuple(robot))
                        .rescued
                    ):
                        self.maze_map[agent_id].maze_view.maze.get_rescue_item(
                            tuple(robot)
                        ).rescued = True
                        # print("rescued")
                        # print()
    ...
    def randomize_rescue_items(self):
        ...
        positions = [(2, 6), (7, 0), (2, 8), (3, 4)]

        for riddle_type, pos in zip(riddle_types, positions):
            position = (
                random.randrange(0, self.maze_size - 1),
                random.randrange(0, self.maze_size - 1),
            )
            while (
                position == (0, 0)
                or position == (9, 9)
                or position in self.rescue_items_dict.keys()
            ):
                position = (
                    random.randrange(0, self.maze_size - 1),
                    random.randrange(0, self.maze_size - 1),
    ...
    def calculate_final_score(self, agent_id, riddlesTimeDictionary):
        rescue_score = (1000 * self.maze_map[agent_id].maze_view.rescued_items) / (
            self.maze_map[agent_id].steps
        )
        riddles_score = 0
        ...
        for riddle in self.riddles_dict[agent_id].riddles.values():
            ...
            if riddle_score > 0:
                riddle_score = riddle_score / (
                    riddlesTimeDictionary.get(riddle.riddle_type, 1) * 100
                )
        ...
        if (
            not tuple(self.maze_map[agent_id].maze_view.robot) == (9, 9)
            or not self.maze_map[agent_id].terminated
        ):
...
class RiddleContainer:
    def __init__(self):
        ...
        self.cipher_riddle = CipherRiddle(
            riddle_type="cipher",
            riddle_dir_path="../riddles/cipher-riddles/riddles.json",
        )
        self.cipher_riddle.load_riddle()

        self.server_riddle = ServerRiddle(
            riddle_type="server",
            riddle_dir_path="../riddles/server-riddles/riddles.json",
        )
        self.server_riddle.load_riddle()

        self.pcap_riddle = PcapRiddle(
            riddle_type="pcap", riddle_dir_path="../riddles/pcap-riddles/riddles.json"
        )
        self.pcap_riddle.load_riddle()

        self.captcha_riddle = CaptchaRiddle(
            riddle_type="captcha",
            riddle_dir_path="../riddles/captchav2-riddles/riddles.json",
        )
